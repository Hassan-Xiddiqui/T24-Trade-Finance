...
//        File file = new File("C:\\Users\\xcelliti2\\IdeaProjects\\Retail Ops\\Reports\\" +testCaseName+ ".png");
    ...
    @BeforeMethod(groups = {"ImportInputter"})
    public void inputterLogin() {
        chromeConfig();

        PageObject.signIn("import1", "QWer1234");

        PageObject.switchFrame(1);

        PageObject.menu_Dropdown("PSW - Import");

    }

    @BeforeMethod(groups = {"IBGImportInputter"})
    public void IBGinputterLogin() {
        chromeConfig();

        PageObject.signIn("ibgimport01", "QWer1234");
    ...
    @BeforeMethod(groups = {"IBGExportInputter"})
    public void IBGExportInputter() {
        chromeConfig();

        PageObject.signIn("ibgexport1", "QWer1234");

        PageObject.switchFrame(1);

        PageObject.menu_Dropdown("PSW - Export");

    }

    @BeforeMethod(groups = {"IBGGuaranteeInputter"})
    public void IBGGuaranteeInputter() {
        chromeConfig();

        PageObject.signIn("ibgguarantee1", "QWer1234");

        PageObject.switchFrame(1);

        PageObject.menu_Dropdown("Guarantees Menu - Islamic");

    }

    @BeforeMethod(groups = {"ExportInputter"})
    public void ExportinputterLogin() {
        chromeConfig();

        PageObject.signIn("export1", "QWer1234");
    ...
    @BeforeMethod(groups = {"GuaranteeInputter"})
    public void GuaranteeInputterLogin() {
        chromeConfig();

        PageObject.signIn("guarantee1", "QWer1234");

        PageObject.switchFrame(1);

        PageObject.menu_Dropdown("Guarantee Menu");
...
//    @AfterMethod(groups = {"Authorizer" , "ImportInputter", "IBGInputter", "IBGAuthorizer"})
